...
def blue_channel_changed(self, context):
    from .session import update_blue_channel
    update_blue_channel(self.id_data)
    if context and context.screen:
        for area in context.screen.areas:
            if area.type == 'VIEW_3D':
                area.tag_redraw()
...
def blue_option(value, description):
    # One stored choice, exposed as three mutually exclusive checkboxes.
    def get(self):
        return self.blue_channel == value

    def set(self, selected):
        if selected:
            self.blue_channel = value

    return BoolProperty(name=value, description=description, get=get, set=set)
...
class WARPFLOW_PG_mesh(bpy.types.PropertyGroup):
    ...
    blue_channel: EnumProperty(name="Blue Channel", items=[('0', '0', 'Black'), ('1', '1', 'White'), ('0.5', '0.5', 'Mid grey')], default='0', update=blue_channel_changed)
    blue_black: blue_option('0', 'Black blue channel')
    blue_white: blue_option('1', 'White blue channel')
    blue_grey: blue_option('0.5', 'Mid grey blue channel')
