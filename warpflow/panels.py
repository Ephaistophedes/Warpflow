...
def draw_blue_channel(layout, obj):
    if obj is None or obj.type != 'MESH':
        return
    column = layout.column(align=True)
    column.enabled = obj.data.is_editable
    column.label(text='Blue Channel')
    row = column.row(align=True)
    for name in ('blue_black', 'blue_white', 'blue_grey'):
        row.prop(obj.data.warpflow, name)
...
class WARPFLOW_PT_paint(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        draw_blue_channel(layout, active.obj if active else obj)
        ...
        row.operator('warpflow.import_guides', icon='IMPORT')
        row.operator('warpflow.export_guides', icon='EXPORT')
        if active:
            layout.label(text='Exit paint mode to import/export guides.', icon='INFO')
        if active:
...
class WARPFLOW_PT_export(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        draw_blue_channel(layout, context.object)
