...
def write_field(mesh, field):
    ...
    rgba[:, 2] = float(mesh.warpflow.blue_channel)
    rgba[:, 3] = 1.0
    attr.data.foreach_set('color', rgba.ravel())
    mesh.update()


def update_blue_channel(mesh):
    """Change only blue, including any currently displayed stroke preview."""
    attr = mesh.color_attributes.get(ATTRIBUTE)
    if attr is None or attr.domain != 'POINT' or attr.data_type != 'FLOAT_COLOR':
        return
    rgba = np.empty((len(mesh.vertices), 4), dtype=np.float32)
    attr.data.foreach_get('color', rgba.ravel())
    rgba[:, 2] = float(mesh.warpflow.blue_channel)
    attr.data.foreach_set('color', rgba.ravel())
    mesh.update()
