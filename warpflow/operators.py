...
class WARPFLOW_OT_import_guides(bpy.types.Operator, ImportHelper):
    bl_idname = 'warpflow.import_guides'
    bl_label = 'Import Guides'
    bl_description = 'Load saved guides onto a matching mesh and rebuild the flow field (undoable)'
    bl_options = {'REGISTER', 'UNDO'}
    filename_ext = '.json'
    filter_glob: StringProperty(default='*.json', options={'HIDDEN'})
    mode: EnumProperty(
        name='Import Mode',
        items=[('APPEND', 'Append', 'Add guides after the existing strokes'),
               ('REPLACE', 'Replace', 'Replace all existing strokes with the imported guides')],
        default='APPEND')
    ...
    def draw(self, context):
        ...
        self.layout.label(text='Requires matching geometry and active UVs.')
        self.layout.label(text='Uses current sharpness and distance settings.')

    def execute(self, context):
        from .guide_io import import_guides
        context.window_manager.progress_begin(0, 1)
        if context.window:
            context.window.cursor_set('WAIT')
        try:
            count = import_guides(context, self.filepath, self.mode,
                                  progress=context.window_manager.progress_update)
            self.report({'INFO'}, f'Imported {count} guides ({self.mode.lower()}).')
            return {'FINISHED'}
        except Exception as exc:
            self.report({'ERROR'}, str(exc))
            return {'CANCELLED'}
...
class WARPFLOW_OT_export_guides(bpy.types.Operator, ExportHelper):
    bl_idname = 'warpflow.export_guides'
    bl_label = 'Export Guides'
    bl_description = 'Save all completed guides, their strengths and mirror links to a JSON file'
    filename_ext = '.json'
    filter_glob: StringProperty(default='*.json', options={'HIDDEN'})
    ...
    def poll(cls, context):
        return (session.ACTIVE is None and context.object is not None
                and context.object.type == 'MESH' and context.object.mode == 'OBJECT'
                and bool(context.object.data.warpflow.strokes))

    def invoke(self, context, event):
        self.filepath = bpy.path.clean_name(context.object.name) + '_guides.json'
        return ExportHelper.invoke(self, context, event)

    def execute(self, context):
        from .guide_io import export_guides
        try:
            path = export_guides(context.object, self.filepath)
            self.report({'INFO'}, f'Exported {len(context.object.data.warpflow.strokes)} guides: {path}')
            return {'FINISHED'}
        except Exception as exc:
            self.report({'ERROR'}, str(exc))
            return {'CANCELLED'}
...
CLASSES = (WARPFLOW_OT_enter, WARPFLOW_OT_paint, WARPFLOW_OT_exit,
           WARPFLOW_OT_delete_stroke, WARPFLOW_OT_clear,
           WARPFLOW_OT_import_guides, WARPFLOW_OT_export_guides, WARPFLOW_OT_export)
